...
contract DemaxPair is BaseShareField {
    ...
    function _increase(address user, uint value) internal {
        _increaseProductivity(user, value);
        address pool = IDemaxPlatform(IDemaxConfig(CONFIG).platform()).POOL();
        IDemaxPool(pool).increaseProdutivity(user, value);
    }

    function _decrease(address user, uint value) internal {
        _decreaseProductivity(user, value);
        address pool = IDemaxPlatform(IDemaxConfig(CONFIG).platform()).POOL();
        IDemaxPool(pool).decreaseProdutivity(user, value);
    }
    ...
    function _transfer(address from, address to, uint value) private {
        ...
        _decrease(from, value);
        _increase(to, value);
        IDemaxFactory(FACTORY).addPlayerPair(to, address(this));
